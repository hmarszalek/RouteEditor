...
class RouteAPITestCase(APITestCase):
    def setUp(self):
        ...
        self.background = BackgroundImage.objects.create(name="Test BG", image="media/backgrounds/mapa-ochoty.jpg")
...
class RoutePointAPITestCase(APITestCase):
    def setUp(self):
        ...
        self.background = BackgroundImage.objects.create(name="BG", image="media/backgrounds/sample.jpg")
        self.route = Route.objects.create(name="Test Route", user=self.user, background=self.background)

    def test_add_point_to_route(self):
        url = f"/api/routes/{self.route.id}/points/"
    ...
    def test_update_point(self):
        ...
        response = self.client.put(f"/api/routes/{self.route.id}/points/{point.id}/", {
            "name": "Updated name",
            "x": 15,
            "y": 25,
            "route": self.route.id
    ...
    def test_unauthorized_point_access(self):
        ...
        self.client.credentials()  # Remove auth
        response = self.client.get(f"/api/routes/{self.route.id}/points/{point.id}/")
