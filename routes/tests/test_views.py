...
class WebViewTests(TestCase):
    ...
    def test_remove_point_view(self):
        route = Route.objects.create(name="With Point", background=self.background, user=self.user)
        point = RoutePoint.objects.create(name="Punkt A", x=10, y=20, user=self.user, route=route)
        response = self.client.get(reverse('remove_point', args=[route.id, point.id]))
        self.assertEqual(response.status_code, 302)
        self.assertEqual(RoutePoint.objects.count(), 0)

    def test_redirect_for_unauthenticated(self):
        self.client.logout()
        response = self.client.get(reverse('my_routes'))
